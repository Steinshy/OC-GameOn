...
const validateRadioGroup = (input) => {
  const checked = Object.values(validationRefs?.radioButtonsInputs || {}).some(radio => radio?.checked);
...
const validateCheckbox = (input) => {
  return setFormFieldState(input.id, !input.checked, input.checked), input.checked;
};
...
const validateField = (input) => {
  ...
  if (input.type === "checkbox") return input.id === "terms_of_service" ? validateCheckbox(input) : true;
